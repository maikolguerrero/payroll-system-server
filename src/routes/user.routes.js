...
// Ruta para contar la cantidad de admins con el rol admin_principal
...
router.get('/search', authMiddleware(['admin_principal']), UserController.searchUsers);

// Rutas protegidas:
router.get('/', authMiddleware(['admin_principal']), UserController.getUsersWithPagination);
router.get('/all', authMiddleware(['admin_principal']), UserController.getAllUsers);
